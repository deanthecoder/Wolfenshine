...
Wolfenshine currently loads and decompresses all 60 maps from the original six-episode data, locates the E1M1 player start, and provides arrow-key navigation through a textured 320×200 software-raycast view using the original indexed wall art, ready-pistol sprite, and VGA palette. The native image is presented at the original 4:3 display aspect ratio, accounting for Mode 13h's non-square pixels. Space opens ordinary sliding doors. World sprites, locked doors, automatic door closing, and wider gameplay are not implemented yet.
